...
def load_data(args):
    ...
    if(args.debug):
        with open("../data_{}/dataset_onhour_sample.pkl".format(args.cityname),"rb") as fp:
            data_xy = pickle.load(fp)
        with open("../data_{}/message_edges_sample.pkl".format(args.cityname),"rb") as fp:
            message_edges = pickle.load(fp)
        data_hour_0 = []
        for xy, edges in zip(data_xy, message_edges):
            data_hour_0.append(xy + edges)
        L = len(data_hour_0)
        ...
        with open("../data_{}/dataset_onhour.pkl".format(args.cityname),"rb") as fp:
            data_xy = pickle.load(fp)
        with open("../data_{}/message_edges.pkl".format(args.cityname),"rb") as fp:
            message_edges = pickle.load(fp)
        data_hour_0 = []
        for xy, edges in zip(data_xy, message_edges):
            data_hour_0.append(xy + edges)
